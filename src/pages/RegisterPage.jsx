...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to BizDir!');
      navigate('/');
    } catch (err) {
      const msg = {
  ...
  return (
    <>
      <Helmet><title>Register – BizDir</title></Helmet>
      <div className="min-h-[calc(100vh-4rem)] bg-surface-50 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-card p-8 border border-surface-100">
            ...
            <h1 className="font-display font-bold text-2xl text-surface-900 text-center mb-1">Create account</h1>
            <p className="text-sm font-body text-surface-400 text-center mb-7">Join BizDir to discover & list businesses</p>

            <form onSubmit={handleSubmit} className="space-y-4">
              <Input label="Full Name" icon={User} value={form.name} onChange={set('name')} placeholder="Ravi Kumar" required />
              <Input label="Email" type="email" icon={Mail} value={form.email} onChange={set('email')} placeholder="you@example.com" required />
              <Input label="Password" type="password" icon={Lock} value={form.password} onChange={set('password')} placeholder="Min 6 characters" required />
              <Input label="Confirm Password" type="password" icon={Lock} value={form.confirm} onChange={set('confirm')} placeholder="Repeat password" required />
